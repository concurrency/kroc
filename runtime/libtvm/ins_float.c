...
TVM_INSTRUCTION (ins_roundsn)
{
	...
	STACK_RET(f, UNDEFINE(BREG), UNDEFINE(CREG), STYPE_DATA, BREGt, CREGt);
}
...
TVM_INSTRUCTION (ins_fmul)
{
	...
	STACK_RET(tmpint_c, CREG, UNDEFINE(CREG), STYPE_DATA, CREGt, CREGt);
}
...
TVM_INSTRUCTION (ins_fpldnlsni)
{
	...
	//printf("AREG %x, BREG %x\n ", AREG, BREG);
	STACK(CREG, UNDEFINE(BREG), UNDEFINE(CREG), CREGt, CREGt, CREGt);
	SET_ROUNDMODE_RET(FE_TONEAREST);
}
