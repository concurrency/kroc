...
/* SDL_eventaction enum causes a problem
should be typemapped to a VAL_INT but is typemapped to INT (so an int* which is no good)*/
...
/* Waits indefinitely for the next available event, returning 1, or 0 if there
   ...
   event is removed from the queue and stored in that area.
 */
//Wait event should be blocking so the runtime donesnt block when there are no events.
%feature("blocking")  SDL_WaitEvent(SDL_Event *) "B" 
