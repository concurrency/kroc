      ...
      (let ([meta (pass-meta h)]
        ...
        (let* ([op (open-output-file (get-output-filename) 'replace)]
          ...
          (letrec ([byte-print
            ...
            (fprintf op "~n-- WICKED SCHEME PROGRAM OUTPUTSES THIS YEAH!~n")
            ...
            (fprintf op "VAL INT memory.size IS ~a:~n" (+ *WS* *VS* *MS*))
      ...
      (let ([meta (pass-meta h)]
        ...
        (let* ([op (open-output-file (get-output-filename) 'replace)]
          ...
          (letrec ([byte-print
            ...
            (fprintf op "~n/* WICKED SCHEME PROGRAM OUTPUTSES THIS YEAH! */~n")
            (fprintf op "#define ws_size  ~a~n" *WS*)
            (fprintf op "#define vs_size  ~a~n" *VS*)
            (fprintf op "#define ms_size  ~a~n" *MS*)
            (fprintf op "#define inst_size ~a~n" (apply + (map length bytes)))
            (fprintf op "#define memory_size ~a~n" (+ *WS* *VS* *MS*))
            (fprintf op "const unsigned char transputercode[] = {~n")
