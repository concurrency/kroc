...
public class OccPlugOptionPaneCmd extends AbstractOptionPane
{
    ...
    */
		...
		skrocCmdName = new JTextField(jEdit.getProperty(
			OccPlugPlugin.OPTION_PREFIX + "skrocCmd"));
		...
		tvmCmdName = new JTextField(jEdit.getProperty(
			OccPlugPlugin.OPTION_PREFIX + "tvmCmd"));
		...
		firmdlCmdName = new JTextField(jEdit.getProperty(
			OccPlugPlugin.OPTION_PREFIX + "firmdlCmd"));
		...
		dllCmdName = new JTextField(jEdit.getProperty(
			OccPlugPlugin.OPTION_PREFIX + "dllCmd"));
		...
		String legoTowerPortOptions[] = {"DEFAULT", "USB", "COM1", "COM2", "COM3", "COM4", "COM5", "COM6"};
		legoTowerPort = new JComboBox(legoTowerPortOptions);
		String legoTowerPortSelected = jEdit.getProperty(OccPlugPlugin.OPTION_PREFIX + "legoTowerPort");
		/*
		for(int i = 0; i < legoTowerPortOptions.length; i++)
		{
			if(legoTowerPortSelected.equals(legoTowerPortOptions)
			legoTowerPort.
		}*/
		legoTowerPort.setSelectedItem(legoTowerPortSelected);
		org.gjt.sp.util.Log.log(org.gjt.sp.util.Log.DEBUG,this,"Got: " + legoTowerPortSelected);
		
		addComponent("Lego tower port:",
			legoTowerPort);
			
		/* Lib Path */
		libPath = new JTextField(jEdit.getProperty(OccPlugPlugin.OPTION_PREFIX + "MasterLibraryPath"));
		addComponent("Library path: ", libPath);
			
		/* SAVE BEFORE COMPILING ? */
		
		boolean saveOnCompileState = jEdit.getBooleanProperty(OccPlugPlugin.OPTION_PREFIX + "saveOnCompile", true);
		
		saveOnCompile = new JCheckBox("Automatically save source files when compiling");
	...
	public void _save()
	{
		...
		jEdit.setProperty(OccPlugPlugin.OPTION_PREFIX + "tvmCmd",
			tvmCmdName.getText());
		jEdit.setProperty(OccPlugPlugin.OPTION_PREFIX + "firmdlCmd",
			firmdlCmdName.getText());
		jEdit.setProperty(OccPlugPlugin.OPTION_PREFIX + "dllCmd",
			dllCmdName.getText());
		jEdit.setProperty(OccPlugPlugin.OPTION_PREFIX + "MasterLibraryPath", libPath.getText());
		/*jEdit.setBooleanProperty(
			"options.occPlug.slinker.optimalPrefixing", 
		...
		jEdit.setBooleanProperty(OccPlugPlugin.OPTION_PREFIX + "saveOnCompile", saveOnCompile.isSelected());
		org.gjt.sp.util.Log.log(org.gjt.sp.util.Log.DEBUG,this,"Set: " + legoTowerPort.getSelectedItem());
	}
